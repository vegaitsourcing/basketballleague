$(function() {
	...
	$('[all-seasons]').on('click', function() {
		const $this = $(this);
		const cssClass = getActiveCssClass();
	...
	$('.statistic-page').on('click', '[data-table-type]', function() {
		const $this = $(this);
		const tableType = $this.data('table-type');
		const activeCssClass = "active";

		$this.siblings().removeClass(activeCssClass);
		$this.addClass(activeCssClass);

		$('[' + tableType + ']').show().siblings('ol').hide();
	});
